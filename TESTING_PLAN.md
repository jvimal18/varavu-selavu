...
| --- | --- | --- | --- | --- |
| **Auth & sessions** | User list is available before login; first-time user with no PIN is routed to setup; first PIN setup succeeds and persists a bcrypt hash; valid PIN login succeeds; invalid PIN, no-PIN login, malformed payload, and non-existent user are rejected with the documented envelope; current PIN is required to change an existing PIN; correct current PIN plus new PIN changes the PIN; logout revokes the current session; `/api/auth/me` returns the current user and rejects no session; missing, legacy user-id, unknown, expired, and revoked cookies are rejected; a session row is created, hashed in storage, last-seen debounce is honored, and revoke-all-other-sessions keeps only the current session; login rate limit at 20/min per IP, five failures/15 minutes per IP, and progressive per-account cooldown at 5/10/15+ failures; login UI countdown reaches zero and enables retry; switching users clears old user state. `Forget PIN` is **NOT IMPLEMENTED** and is tracked as PR 7/v1.6.1, not a live scenario. | **partial** — 24 pure/SQL tests cover token format, debounce, schema, and revocation, but not real auth HTTP or UI | PR 2 | `auth/http-flows.test.ts`, `auth/rate-limits.test.ts`, `auth/login-state-ui.test.ts`, and existing moved token/schema/revocation tests; real cookie and DB row assertions |
...
| **Backup & operations** | JSON export is complete and versioned for v1.0, v1.1, and v1.2; import accepts all three versions with documented defaults for missing fields; FK-safe wipe/insert order preserves users, accounts, categories, settings, transactions, and sessions; export API matches script output; binary backup is a valid SQLite copy containing schema, data, WAL state, and `__drizzle_migrations`; source/copy integrity failures fail loudly; cleanup deletes sessions expired more than 30 days or revoked more than 7 days, and retains rows exactly at each cutoff; version metadata is generated from the changelog; runtime DB uses WAL, foreign keys ON, and synchronous NORMAL. | **partial** — 9 script tests cover current binary/JSON writes, but not import compatibility, API parity, cleanup cutoffs, or runtime composition | PR 3 | `backup/*.test.ts`, `migrations/runtime-pragmas.test.ts`, and `pwa/version-metadata.test.ts`; fixture round trips, exact row counts/fields, binary integrity, cutoff timestamps, and API/script parity |
| **Security** | Actual CSRF middleware checks state-changing `/api/*` methods (`POST`, `PATCH`, `PUT`, `DELETE`) with exact allowed Origin; accepts configured origin and rejects missing, foreign, partial, scheme-mismatched, and production-empty allowlists; skips safe methods and the explicitly allowed `/api/auth/*` bootstrap paths; global auth gate returns 401 for protected routes and allows auth routes; security headers appear on success, 401, 403, and 500; CSP report-only/enforce mode follows config; rate limiter keys and cooldowns are correct; all login errors expose the documented `data.message` and `data.data.retryAfter` shape without leaking details. | **partial** — 17 CSRF and 11 CSP helper tests, but no actual middleware path, headers-on-errors, or error-envelope coverage | PR 2 | `security/middleware-http.test.ts`, `security/security-headers-http.test.ts`, `security/error-envelope.test.ts`, moved helper tests, and real Origin/Cookie requests |
...
1. **Auth HTTP**: list users without exposing PIN hashes; first setup for a
   no-PIN user; valid and invalid login; missing/malformed/non-existent user;
...
3. **Rate limits**: permit the configured 20 requests/minute threshold and
   reject the next request; block after five failures/15 minutes; assert
   progressive account cooldown at 5, 10, and 15+ failures; verify
   `retryAfter` is present only where the client needs it and expiry permits a
   retry.
...
5. **Headers and errors**: assert CSP mode plus HSTS, nosniff, frame, referrer,
   permissions, and related headers on success and 401/403/500 responses.
   Assert the login error envelope at 400/401/429, including human text in
   `data.message` and lockout seconds in `data.data.retryAfter`.
6. **Migration boundary**: copy the frozen v1.5 DB, run the real migrator, and
   ...
   owner.
7. **Runtime DB**: boot the app against a unique file and assert
   `journal_mode=WAL`, `foreign_keys=1`, and `synchronous=NORMAL` before a
